...
class ClassInSDK {
     ...
     * @param {string|number|Date} params.beginTime - 开始时间（可选）
     * @param {string|number|Date} params.endTime - 结束时间（可选）
     * @param {number} params.teacherUid - 老师UID（可选）
     * @param {string} params.teacherName - 老师姓名（可选）
     * @param {number} params.record - 是否录课（可选）
     * @param {number} params.live - 是否直播（可选）
     * @param {number} params.replay - 是否回放（可选）
    ...
    async updateClassroom(params) {
        ...
        // 标准化时间（如果提供）
        if (updateParams.beginTime) {
            updateParams.beginTime = normalizeTime(updateParams.beginTime);
        }
