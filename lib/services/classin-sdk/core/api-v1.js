    ...
    async request(action, params) {
        ...
        if (result.error_info) {
            if (result.error_info.errno === 1 || result.error_info.errno === 135) {
                ...
                if (this.debug) {
                    console.error('[API v1] error_info:', JSON.stringify(result.error_info));
                }
                const err = new Error(`ClassIn API 错误 (${result.error_info.errno}): ${result.error_info.error || '未知错误'}`);
                err.errno = result.error_info.errno;
                throw err;
            }
        } else {
            return result;
