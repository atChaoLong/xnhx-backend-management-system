...
const logger = createLogger('SignupClient')
...
export function SignupClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.warn('注册请求失败', summarizeError(err))
      setError(getDisplayErrorMessage(err, "注册失败，请重试"))
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-2xl font-bold">注册申请已提交</CardTitle>
              <CardDescription className="mt-2">
                请联系管理员分配角色后再登录，正在跳转到登录页...
              </CardDescription>
            </div>
          </CardHeader>
        </Card>
