...
import { Edit, Loader2, RefreshCw, Upload } from "lucide-react"
...
import InterviewVideoForm from "@/components/teacher/recruitment/InterviewVideoForm"
...
export default function TeacherCandidateUploadPage() {
  ...
  const canUploadVideo = teacherCandidates.uploadVideo()
  ...
  async function fetchCandidates(page: number = 1, size: number = pageSize) {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "加载失败",
        description: error.message || "无法加载待上传录像候选人",
      })
    } finally {
      setIsLoading(false)
    }
  ...
  if (!canUploadVideo) {
    return (
      <div className="flex flex-col h-full">
        <Header title="初试录像上传" description="上传已约面候选人的面试录像" />
        ...
        <Card className="h-full flex flex-col">
          <CardContent className="flex-1 flex flex-col p-6 overflow-hidden">
            <div className="flex items-center justify-between gap-4 mb-6 flex-shrink-0">
              <div>
                <h3 className="text-lg font-semibold">待上传录像列表</h3>
                <PaginationInfo
              ...
              <Table className="border-0">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>约面人</TableHead>
                    <TableHead>面试链接</TableHead>
                  ...
                  {candidates.length === 0 ? (
                    ...
                    candidates.map((candidate) => (
                      <TableRow key={candidate.id}>
                        ...
                        <TableCell>{candidate.interview_officer || candidate.interviewer_name || "-"}</TableCell>
