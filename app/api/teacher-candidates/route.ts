...
// GET: 获取教师候选列表（支持ID查询单个）
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url)
    const id = searchParams.get('id')

    logger.debug('获取教师候选数据', { id })
    ...
    if (id) {
      ...
      if (error) {
        logger.error('获取教师候选失败', { id, message: error.message, code: error.code })
    ...
    if (error) {
      logger.error('获取教师候选列表失败', { message: error.message, code: error.code })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.debug('获取教师候选列表成功', { count: data?.length || 0 })
    return NextResponse.json({ data })
  } catch (error: any) {
    logger.error('获取教师候选异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '获取教师候选失败' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.debug('创建教师候选 - 接收到的数据', { body })

    // 后端验证：必填字段
    if (!body.name || typeof body.name !== 'string' || !body.name.trim()) {
      logger.error('创建教师候选失败 - 姓名为空', { body })
      return NextResponse.json(
        { error: '姓名不能为空' },
        { status: 400 }
      )
    ...
    if (!body.wechat_id || typeof body.wechat_id !== 'string' || !body.wechat_id.trim()) {
      logger.error('创建教师候选失败 - 微信号为空', { body })
      return NextResponse.json(
        { error: '微信号不能为空' },
        { status: 400 }
      )
    ...
    const insertData = {
      ...
      appointment_week: body.appointment_week || null,
      registration_date: body.registration_date || null,
      interview_score: body.interview_score || null,
      interview_score_total: body.interview_score_total || null,
      ...
      interview_evaluation: body.interview_evaluation || null,
      ...
      enrolled_teacher_name: body.enrolled_teacher_name || null,
    ...
    logger.debug('创建教师候选 - 准备插入的数据', { insertData })
    ...
    if (error) {
      logger.error('创建教师候选失败', { message: error.message, code: error.code, details: error.details })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.info('创建教师候选成功', { id: data.id, name: data.name })
    return NextResponse.json({ data }, { status: 201 })
  } catch (error: any) {
    logger.error('创建教师候选异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '创建教师候选失败' },
      { status: 500 }
    )
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    logger.debug('更新教师候选 - 接收到的数据', { id, updateData })

    // 后端验证：必填字段
    if (updateData.name !== undefined && (!updateData.name || !updateData.name.trim())) {
      logger.error('更新教师候选失败 - 姓名为空', { id, updateData })
      return NextResponse.json(
        { error: '姓名不能为空' },
        { status: 400 }
      )
    ...
    if (updateData.wechat_id !== undefined && (!updateData.wechat_id || !updateData.wechat_id.trim())) {
      logger.error('更新教师候选失败 - 微信号为空', { id, updateData })
      return NextResponse.json(
        { error: '微信号不能为空' },
        { status: 400 }
      )
    ...
    const optionalFields = [
      ...
      'interview_month', 'interview_week', 'appointment_week', 'registration_date',
      'interview_score', 'interview_score_total', 'logical_expression_score',
      'dress_appearance_score', 'material_preparation_score', 'exam_score',
      'initial_evaluation', 'interview_evaluation', 'teacher_characteristics',
      'mandarin_level', 'research_ability', 'service_awareness', 'affinity',
      'review_status', 'reviewer_name', 'review_result', 'review_evaluation_comment',
      'review_date', 'reviewed_by', 'teacher_level', 'can_teach_graduation_class',
      'is_hired', 'teacher_feeling', 'suitable_for_students', 'scheduling_preference',
      'enrolled_teacher_name', 'hired_notes', 'qr_code', 'current_rate', 'approved_hourly_rate'
    ]
    ...
    logger.debug('更新教师候选 - 准备更新的数据', { id, updatePayload })
    ...
    if (error) {
      logger.error('更新教师候选失败', { id, message: error.message, code: error.code })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.info('更新教师候选成功', { id, name: data.name })
    return NextResponse.json({ data })
  } catch (error: any) {
    logger.error('更新教师候选异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '更新教师候选失败' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('删除教师候选失败', { id, message: error.message, code: error.code })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.info('删除教师候选成功', { id })
    return NextResponse.json({ success: true })
  } catch (error: any) {
    logger.error('删除教师候选异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '删除教师候选失败' },
      { status: 500 }
    )
  }
}
