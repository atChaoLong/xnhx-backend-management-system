...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      ...
      if (error) {
        logger.error('获取微信号失败', { id, message: error.message, code: error.code })
    ...
    if (error) {
      logger.error('获取微信号列表失败', { message: error.message, code: error.code })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.error('获取微信号异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '获取微信号失败' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.wechat_name || typeof body.wechat_name !== 'string' || !body.wechat_name.trim()) {
      logger.error('创建微信号失败 - 微信昵称为空', { body })
      return NextResponse.json(
        { error: '微信昵称不能为空' },
        { status: 400 }
      )
    ...
    if (!body.account_type || typeof body.account_type !== 'string' || !body.account_type.trim()) {
      logger.error('创建微信号失败 - 账号类型为空', { body })
      return NextResponse.json(
        { error: '账号类型不能为空' },
        { status: 400 }
      )
    ...
    if (!body.phone || typeof body.phone !== 'string' || !body.phone.trim()) {
      logger.error('创建微信号失败 - 手机号为空', { body })
      return NextResponse.json(
        { error: '手机号不能为空' },
        { status: 400 }
      )
    ...
    if (!body.login_password || typeof body.login_password !== 'string' || !body.login_password.trim()) {
      logger.error('创建微信号失败 - 登录密码为空', { body })
      return NextResponse.json(
        { error: '登录密码不能为空' },
        { status: 400 }
      )
    ...
    if (!body.payment_password || typeof body.payment_password !== 'string' || !body.payment_password.trim()) {
      logger.error('创建微信号失败 - 支付密码为空', { body })
      return NextResponse.json(
        { error: '支付密码不能为空' },
        { status: 400 }
      )
    ...
    if (!body.real_name_person || typeof body.real_name_person !== 'string' || !body.real_name_person.trim()) {
      logger.error('创建微信号失败 - 实人名为空', { body })
      return NextResponse.json(
        { error: '实名人不能为空' },
        { status: 400 }
      )
    ...
    logger.debug('创建微信号 - 准备插入的数据', { insertData })
    ...
    if (error) {
      logger.error('创建微信号失败', { message: error.message, code: error.code, details: error.details })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.error('创建微信号异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '创建微信号失败' },
      { status: 500 }
    )
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const optionalFields = [
      ...
      'team', 'account_type', 'phone', 'login_password', 'payment_password',
      'real_name_person', 'status'
    ]
    ...
    logger.debug('更新微信号 - 准备更新的数据', { id, updatePayload })
    ...
    if (error) {
      logger.error('更新微信号失败', { id, message: error.message, code: error.code })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.error('更新微信号异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '更新微信号失败' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('删除微信号失败', { id, message: error.message, code: error.code })
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
    ...
    logger.error('删除微信号异常', { message: error.message, stack: error.stack })
    return NextResponse.json(
      { error: error.message || '删除微信号失败' },
      { status: 500 }
    )
  }
}
